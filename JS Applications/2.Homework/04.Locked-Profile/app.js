async function lockedProfile() {
      ...
      Object.entries(data).forEach((el, i) => {
        ...
        div.innerHTML = `
            <img src="./iconProfile2.png" class="userIcon" />
				...
				<hr>
				<label>Username</label>
				<input type="text" name="user${i + 1}Username" value="${
          el[1].username
        }" disabled readonly />
				<div id ="user${i + 1}HiddenFields">
					<hr>
					<label>Email:</label>
					<input type="email" name="user${i + 1}Email" value="${
          el[1].email
        }" disabled readonly />
					<label>Age:</label>
					<input type="email" name="user${i + 1}Age" value="${
          el[1].age
        }" disabled readonly />
				</div>
        ...
        main.appendChild(div);
        document.getElementById(`user${i + 1}HiddenFields`).style.display =
          "none";
      });
    }
  } catch (error) {
  ...
  buttons.forEach((button, i) => {
    button.addEventListener(
      "click",
      (func = () => {
        const lock = document.getElementsByName(`user${i + 1}Locked`)[0]
          .checked;
