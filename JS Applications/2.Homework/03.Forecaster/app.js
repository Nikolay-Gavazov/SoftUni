async function attachEvents() {
    ...
    async function submit(){
        ...
        try {
            const responce = await fetch(`http://localhost:3030/jsonstore/forecaster/today/${cityCode}`);
            if(responce.ok == false) throw Error;
            const data = await responce.json();
        ...
        try {
            const responce = await fetch(`http://localhost:3030/jsonstore/forecaster/upcoming/${cityCode}`);
            if(responce.ok == false) throw Error;
            const data = await responce.json();
            const div = document.createElement('div');
