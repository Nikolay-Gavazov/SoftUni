class CarDealership {
    ...
    currentCar() {
        ...
        const result = this.availableCars.map(x => `---${x.model} - ${x.horsepower} HP - ${x.mileage.toFixed(2)} km - ${x.price.toFixed(2)}$`);
    ...
    salesReport(criteria) {
        ...
        const result = this.soldCars.map(x => `---${x.model} - ${x.horsepower} HP - ${x.soldPrice.toFixed(2)}$`);
