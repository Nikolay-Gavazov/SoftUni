...
describe("chooseYourCar", function () {
  it("check for car invalid info", () => {
    assert.equal(
      chooseYourCar.choosingType("Sedan", "Color", 1899),
      "Invalid Year!"
    );
    assert.equal(
      chooseYourCar.choosingType("Sedan", "Color", 2023),
      "Invalid Year!"
    );
  });
  it("check for car", () => {
    assert.equal(
      chooseYourCar.choosingType("coupe", "Color", 2020),
      "This type of car is not what you are looking for."
    );
    assert.equal(
      chooseYourCar.choosingType("Sedan", "Color", 2010),
      "This Color Sedan meets the requirements, that you have."
    );
    assert.equal(
      chooseYourCar.choosingType("Sedan", "Color", 2013),
      "This Color Sedan meets the requirements, that you have."
    );
    assert.equal(
      chooseYourCar.choosingType("Sedan", "Color", 2000),
      "This Sedan is too old for you, especially with that Color color."
    );
  });
  it("check brand name wrong input", () => {
    ...
    assert.equal(
      chooseYourCar.brandName(["a", "b", "c"], 1.5),
      "Invalid Information!"
    );
  });
  ...
  it("check consum ", () => {
    ...
    assert.equal(
      chooseYourCar.carFuelConsumption(1000, 65),
      "The car is efficient enough, it burns 6.50 liters/100 km."
    );
    assert.equal(
      chooseYourCar.carFuelConsumption(100, 20),
      "The car burns too much fuel - 20.00 liters!"
    );
    assert.equal(
      chooseYourCar.carFuelConsumption(40, 9),
      "The car burns too much fuel - 22.50 liters!"
    );
  });
  ...
  it("check consum wrong input", () => {
    ...
    assert.equal(
      chooseYourCar.carFuelConsumption(-5, 10),
      "Invalid Information!"
    );
    assert.equal(
    ...
    assert.equal(
      chooseYourCar.carFuelConsumption(0, 10),
      "Invalid Information!"
    );
