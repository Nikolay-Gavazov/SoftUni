...
describe("findNewApartment", function () {
  it("check location wrong input", () => {
    ...
    assert.equal(
      findNewApartment.isGoodLocation(true, false),
      "Invalid input!"
    );
    assert.equal(
      findNewApartment.isGoodLocation(["p"], true),
      "Invalid input!"
    );
  });
  it("check location", () => {
    assert.equal(
      findNewApartment.isGoodLocation("Burgas", true),
      "This location is not suitable for you."
    );
    assert.equal(
      findNewApartment.isGoodLocation("Vienna", false),
      "This location is not suitable for you."
    );
    assert.equal(
  ...
  it("check area", () => {
    assert.equal(
      findNewApartment.isLargeEnough([50, 60, 70, 80], 65),
      "70, 80"
    );
    assert.equal(
      findNewApartment.isLargeEnough([50, 60, 70, 80], 40),
      "50, 60, 70, 80"
    );
  ...
  it("check budged", () => {
    ...
    assert.equal(
      findNewApartment.isItAffordable(2, 2),
      "You can afford this home!"
    );
    assert.equal(
      findNewApartment.isItAffordable(2, 3),
      "You can afford this home!"
    );
    assert.equal(
      findNewApartment.isItAffordable(2, 1),
      "You don't have enough money for this house!"
    );
    assert.equal(
      findNewApartment.isItAffordable(3, 2),
      "You don't have enough money for this house!"
    );
  });
});
